...
$conn -> close();
   ...
   $conn -> autocommit(FALSE);
    ...
    $sql="SELECT * from metodelab where kdmetode='$kdmetode' and kdcabang='$kdcabang'";
...
if($rowcount > 0){
$conn -> query("UPDATE metodelab set metode='$namatamplates' 
where kdcabang='$kdcabang' and status='$status' and kdmetode='$kdmetode'");


}else{
    
      $conn -> query("INSERT INTO metodelab(kdmetode , metode , kdcabang,status) 
 values('$namatamplates','$namatamplates','$kdcabang','$status')");
