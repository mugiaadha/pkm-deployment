...
if(empty($data->catatandiet)){
$catatandiet='';
}else{

$catatandiet=str_replace("'"," ` ",$data->catatandiet);
...
if(empty($data->riwayatkeluarga)){
  ...
  // `alergiudara` VARCHAR(600) NULL DEFAULT NULL COLLATE 'latin1_swedish_ci',
  // `alergiobat` VARCHAR(600) NULL DEFAULT NULL COLLATE 'latin1_swedish_ci',
  // `kdprognosa` VARCHAR(600) NULL DEFAULT NULL COLLATE 'latin1_swedish_ci',
  // `terapinonobat` VARCHAR(600) NULL DEFAULT NULL COLLATE 'latin1_swedish_ci'
...
,alergiudara='$data->alergiudara',alergiobat='$data->alergiobat',catatandiet='$catatandiet',kdprognosa='$data->kdprognosa',
terapinonobat='$data->terapinonobat',lp='$data->lingkarperut'
...
if($hakakses === 'Perawat'){
 ...
 '$data->alergiudara','$data->alergiobat','$data->lingkarperut')");
...
status='SIAP' where notransaksi='$notrans' and
 kdcabang='$kdcabang' and status='ANTRI'");
       ...
       $conn -> query("INSERT INTO ermcppt(kduser,notrans,tgl,norm,kddokter,kdpoli,subjek,td,bb,nadi,suhu,rr,spo,pf,planing,kdcabang,alergi,tdd,hr,tb,rwytp,subjekp,
    tglkontrol,rencanatindakan,stspulang,skalanyeri,imt,riwayatdahulu,riwayatkeluarga
  ,alergiudara,alergiobat,kdprognosa,terapinonobat,lp
    ) 
 ...
 '$tglkontrol','$rencanatindakan','$stspulang','$skalanyeri','$imt','$riwayatdahulu',
 '$riwayatkeluarga','$data->alergiudara','$data->alergiobat','$data->kdprognosa','$data->terapinonobat','$data->lingkarperut')");
...
$waktu = date("Y-m-d H:i:s");
    ...
    $sqlx="SELECT * from noanteanfarmasi where tgl='$tgl'   ";
    $resultx=mysqli_query($conn,$sqlx);
     ...
     $rowcountx=mysqli_num_rows($resultx);
        ...
        if($rowcountx > 0){
          ...
          $sqlxl="SELECT * from noanteanfarmasi where tgl='$tgl' order by no desc limit 1   ";
          $resultxl=mysqli_query($conn,$sqlxl);
        
        while($row=mysqli_fetch_array($resultxl,MYSQLI_ASSOC)) {
...
$noantrean = 1;
  ...
  $conn -> query("INSERT INTO noanteanfarmasi(notrans,no,status,tgl,waktu) 
 values('$notrans','$noantrean','0','$tgl','$waktu')");
 
 

}else{
  $conn -> query("delete from  noanteanfarmasi  where notrans='$notrans'");
