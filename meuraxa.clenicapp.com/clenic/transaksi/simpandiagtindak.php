...
if($stssimpan === '1'){
 ...
 values('$tgl','$notrans','$norm','$kddokter','$kdpoli','$kddiagnosa','$diagnosa','$status','$kdcabang','$angka')");
  ...
  $alerttacc=$data->alerttacc;


  
  if($alerttacc === '1'){
    $sqlsaldov="SELECT * from dignosatolak where kdiag='$kddiagnosa'";

  $resultsaldov=mysqli_query($conn,$sqlsaldov);
   $rowcountsaldov=mysqli_num_rows($resultsaldov);
    
  if($rowcountsaldov < 1 ){
          $conn -> query("INSERT INTO dignosatolak(kdiag,diag) 
 values('$kddiagnosa','$diagnosa')");
