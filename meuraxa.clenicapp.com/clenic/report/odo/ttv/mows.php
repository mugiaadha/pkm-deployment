...
$sqlsd = "SELECT top 1 *  FROM  ERMEWSHASILFIXP where  notrans='$notrans' and nolembar='$nolembar' order by tanggal desc";
...
<svg height="1300" width="991" style="background: url(mows.jpg) no-repeat 0 0;border: 1px solid black" >
...
$sqlsd = "SELECT * FROM  ERMEWSTGLP where  notrans='$notrans' and nolembar='$nolembar' ";
 $stmtsd = sqlsrv_query( $conn, $sqlsd );
...
$sqlsd = "SELECT * FROM  ERMEWSWAKTUP where  notrans='$notrans' and nolembar='$nolembar' order by jamx";
 $stmtsd = sqlsrv_query( $conn, $sqlsd );
while( $rowsd = sqlsrv_fetch_array( $stmtsd, SQLSRV_FETCH_ASSOC) ) {
...
<!-- RR -->
 ...
 <polyline points=
"<?php
$sqls = "SELECT * FROM  ERMEWSHASILFIXP where  notrans='$notrans' and nolembar='$nolembar' order by rrx ";
...
<!-- <circle cx='103' cy='1086' r='4'   fill='black' />
<circle cx='103' cy='1104' r='4'   fill='black' />
...
$sqlsd = "SELECT * FROM  ERMEWSHASILFIXP where  notrans='$notrans' and nolembar='$nolembar' order by rrx ";
 $stmtsd = sqlsrv_query( $conn, $sqlsd );
while( $rows = sqlsrv_fetch_array( $stmtsd, SQLSRV_FETCH_ASSOC) ) {
...
$eskalasix = $rows['eskalasix']+5;
$eskalasiy = $rows['eskalasiy']+5;
...
echo "<text x='".$ekgx."' y='".$ekgy."' fill='red' style='font-size: 12px;'>".$rows['ekg']."</text>";

echo "<text x='".$scanx."' y='".$scany."' fill='red' style='font-size: 12px;'>".$rows['scan']."</text>";

echo "<text x='".$eskalasix."' y='".$eskalasiy."' fill='red' style='font-size: 12px;'>".$rows['eskalasi']."</text>";
