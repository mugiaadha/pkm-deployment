...
function login() {
    ...
    try {
        ...
        $db = getDB();
        $userData ='';
        $sql = "SELECT *  FROM user WHERE username=:username and password=:password and kdcabang=:kdcabang ";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("username", $data->username, PDO::PARAM_STR);
       $stmt->bindParam("kdcabang", $data->kdcabang, PDO::PARAM_STR);
        ...
        $tglsimpan = date("YmdH:i:s");
        
        if(!empty($userData))
        {
            $user_id=$userData->user_id;
            $userData->token = apiToken($user_id.$tglsimpan);
        }
...
function ceklogin() {
    ...
    try {
        ...
        $db = getDB();
        $userData ='';
        $sql = "SELECT *  FROM user WHERE username=:username and password=:password and kdcabang='003' ";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("username", $data->username, PDO::PARAM_STR);
